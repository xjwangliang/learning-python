...
def validParam(*varargs, **keywords):
    '''验证参数的装饰器。'''
    print ">>In validParam,varargs:",varargs," keywords:",keywords
    ...
    def generator(func):
        ...
        dctValidator = _getcallargs(args, varargname, kwname,
                                    varargs, keywords)
        print ">>In generator,func is ",func.__name__," dctValidator:",dctValidator
        def wrapper(*callvarargs, **callkeywords):
            dctCallArgs = _getcallargs(args, varargname, kwname,
                                       callvarargs, callkeywords)
            print ">>In wrapper,func is ",func.__name__," dctCallArgs:",dctCallArgs
            k, item = None, None
            try:
...
def _unittest(func, *cases):
    print "IN _unittest,function name ",func.__name__," paramters cases ",cases
    for case in cases:
        print "paramter ",case
        _functest(func, *case)
...
def _functest(func, isCkPass, *args, **kws):
    print "IN _functest, function name ",func.__name__," paramters isCkPass ",isCkPass," args ",args," kws ",kws
    if isCkPass:
        func(*args, **kws)
        print "IN _functest, call ",func.__name__,"(",args,kws,") success"
    else:
        try:
            print "IN _functest, call ",func.__name__
            func(*args, **kws)
            assert False
        except ValidateException:
            print "IN _functest, call ",func.__name__,"(",args,kws,") exception"
            pass
...
#            print ">>>",f," is startswith '_test' "
#            f = d[f]
#            if isinstance(f, FunctionType):
#                print ">>>",f.__name__," is isinstance FunctionType "
#                f()
# 
